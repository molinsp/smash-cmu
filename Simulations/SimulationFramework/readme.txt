...
they will have and and plugins to simulate parts of it in V-Rep. The projects are:
 ...
 - Initial Setup
	...
	V-REP has to be installed in order to be able to use the plugins. V-REP can be obtained from 
	http://www.v-rep.eu/downloads.html. Please set the environment variable VREP_HOME to the root 
	directory of your V-REP installation (where the V-REP executable that starts the simulator 
	is housed). This directory will be where all build	processes copy plugins to.
	...
	Finally, all components use Madara to communicate with each other. The installers for Madara can be used
	to set it up. They can be obtained form https://code.google.com/p/madara/downloads/list

	Once Perl and ACE are setup, the following batch file can generate the projects for Visual Studio 2010:
		generate_vs2010.bat

	Similar commands to the one found inside that batch file can be used to generate projects for other platforms.
